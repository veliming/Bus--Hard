...
static int32_t _core_http_recv(core_http_handle_t *http_handle, uint8_t *buffer, uint32_t len, uint32_t timeout_ms)
{
    ...
    if (http_handle->network_handle != NULL) {
        ...
        if (res < STATE_SUCCESS) {
            ...
            res = _core_http_sysdep_return(res, STATE_SYS_DEPEND_NWK_CLOSED);
        }
    } else {
...
static int32_t _core_http_connect(core_http_handle_t *http_handle)
{
    ...
    if ((res = http_handle->sysdep->core_sysdep_network_setopt(http_handle->network_handle, CORE_SYSDEP_NETWORK_SOCKET_TYPE,
            ...
            (res = http_handle->sysdep->core_sysdep_network_setopt(http_handle->network_handle, CORE_SYSDEP_NETWORK_PORT,
                    &http_handle->port)) < 0 ||
            (res = http_handle->sysdep->core_sysdep_network_setopt(http_handle->network_handle, CORE_SYSDEP_NETWORK_CONNECT_TIMEOUT_MS,
...
static int32_t _core_http_send_auth(core_http_handle_t *http_handle)
{
    ...
    char *path_fmt = "/auth?_v=%s";
    char *path_src[] = { CORE_AUTH_SDK_VERSION };
    ...
    res = core_sprintf(http_handle->sysdep, &path, path_fmt, path_src, sizeof(path_src)/sizeof(char *), CORE_HTTP_MODULE_NAME);
    if (res < STATE_SUCCESS) {
...
static void _core_http_report_version(core_http_handle_t *http_handle)
{
    int32_t res = STATE_SUCCESS;
    char *path = NULL, *header = NULL;
    char *header_src[] = {NULL, NULL};
    char *path_src[] = { http_handle->product_key, http_handle->device_name };
    uint64_t timenow_ms = http_handle->sysdep->core_sysdep_time();
    core_http_request_t request;
    ...
    res = core_sprintf(http_handle->sysdep, &path, "/topic/sys/%s/%s/thing/deviceinfo/update", path_src,
                       sizeof(path_src) / sizeof(char *), CORE_HTTP_MODULE_NAME);
    if (res < STATE_SUCCESS) {
        return;
    }
    ...
    request.content = (uint8_t *)"{\"id\":\"1\",\"version\":\"1.0\",\"params\":["
                      "{\"attrKey\":\"SYS_LP_SDK_VERSION\",\"attrValue\":\""CORE_AUTH_SDK_VERSION"\",\"domain\":\"SYSTEM\"},"
                      "{\"attrKey\":\"SYS_SDK_LANGUAGE\",\"attrValue\":\"C\",\"domain\":\"SYSTEM\"}]}";
    request.content_len = (uint32_t)strlen((const char *)request.content);

    core_http_setopt(http_handle, CORE_HTTPOPT_USERDATA, (void *)&response);
    res = core_http_send(http_handle, &request);
    http_handle->sysdep->core_sysdep_free(path);
...
int32_t aiot_http_auth(void *handle)
{
    ...
    core_http_setopt(http_handle, CORE_HTTPOPT_RECV_HANDLER, (void *)_core_http_auth_recv_handler);
    core_http_setopt(http_handle, CORE_HTTPOPT_USERDATA, (void *)&response);
    if (http_handle->network_handle == NULL ||
    ...
    _core_http_report_version(http_handle);
...
int32_t aiot_http_recv(void *handle)
{
    ...
    memset(&response, 0, sizeof(core_http_response_t));
    core_http_setopt(http_handle, CORE_HTTPOPT_RECV_HANDLER, (void *)_core_http_recv_handler);
    core_http_setopt(http_handle, CORE_HTTPOPT_USERDATA, (void *)&response);
    ...
    if (res < STATE_SUCCESS) {
        ...
        res = STATE_HTTP_RECV_NOT_FINISHED;
    }
