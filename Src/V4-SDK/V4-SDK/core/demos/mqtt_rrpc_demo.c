    ...
    case AIOT_MQTTRECV_PUB: {
        ...
        /* {
            ...
            aiot_mqtt_buff_t topic_buff = {
                .buffer = (uint8_t *)packet->data.pub.topic,
                .len = packet->data.pub.topic_len
            };
            aiot_mqtt_buff_t payload_buff = {
                .buffer = (uint8_t *)payload,
                .len = (uint32_t)strlen(payload)
            };
            aiot_mqtt_pub(handle, &topic_buff, &payload_buff, 0);
        } */
    }
    break;
...
int main(int argc, char *argv[])
{
    ...
    cred.option = AIOT_SYSDEP_NETWORK_CRED_SVRCERT_RSA; /* 使用RSA证书校验MQTT服务端 */
    ...
    */
    ...
    /* {
        ...
        res = aiot_mqtt_pub(mqtt_handle, &pub_topic_buff, &pub_payload_buff, 0);
        if (res < 0) {
            printf("aiot_mqtt_sub failed, res: -0x%04X\n", -res);
            return -1;
        }
