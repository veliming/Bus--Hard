...
typedef enum {
     ...
     *
     * 3. 若@ref aiot_sysdep_network_cred_t 中option配置为@ref AIOT_SYSDEP_NETWORK_CRED_SVRCERT_RSA , MQTT将以tls方式建联
     *
 ...
 *
 * 1. 若配置了@ref AIOT_MQTTOPT_USERNAME , @ref AIOT_MQTTOPT_PASSWORD 和@ref AIOT_MQTTOPT_CLIENTID ,
 *
 *    该接口会直接使用配置的建联参数连接mqtt服务器
 *
 * 2. 若未配置以上参数, 但配置了@ref AIOT_MQTTOPT_PRODUCT_KEY , @ref AIOT_MQTTOPT_DEVICE_NAME 和@ref AIOT_MQTTOPT_DEVICE_SECRET ,
 *
 *    该接口会自动计算连接阿里云物联网平台需要的连接凭证, 并尝试与阿里云物联网平台建立连接
 *
 * @param[in] handle MQTT实例句柄
 *
 * @return int32_t
...
/**
 * @brief 发送一条mqtt PUBLISH报文到MQTT服务器, 用于发布指定的消息
 *
 * @param[in] handle MQTT实例句柄
 * @param[in] topic 指定MQTT PUBLISH报文的topic, 更多信息请参考@ref aiot_mqtt_buff_t
 * @param[in] payload 指定MQTT PUBLISH报文的payload, 更多信息请参考@ref aiot_mqtt_buff_t
 * @param[in] qos 指定MQTT PUBLISH报文的QoS级别, 支持QoS0和QoS1
 *
 * @return int32_t
 * @retval <STATE_SUCCESS 执行失败, 更多信息请参考@ref aiot_state_api.h
 * @retval >=STATE_SUCCESS 执行成功
 ...
 *
 * @param[in] handle MQTT实例句柄
 * @param[in] topic 指定MQTT SUBSCRIBE报文的topic, 更多信息请参考@ref aiot_mqtt_buff_t
 ...
 * @param[in] qos 指定MQTT SUBSCRIBE报文的QoS级别, 支持QoS0和QoS1
 ...
 *
 * @param[in] handle MQTT实例句柄
 * @param[in] topic 指定MQTT UNSUBSCRIBE报文的topic, 更多信息请参考@ref aiot_mqtt_buff_t
 *
 * @return int32_t
 * @retval <STATE_SUCCESS 执行失败, 更多信息请参考@ref aiot_state_api.h
 * @retval >=STATE_SUCCESS 执行成功
 ...
 * 此函数包含了重连机制:
 *
 * 1. 当MQTT心跳丢失超过@ref AIOT_MQTTOPT_HEARTBEAT_MAX_LOST 配置的次数时, 触发重连机制, 重连间隔由
 *
 *    @ref AIOT_MQTTOPT_RECONN_INTERVAL_MS 指定
 *
 * 2. 当SDK检测到网络断开时, 触发重连机制, 重连间隔由 @ref AIOT_MQTTOPT_RECONN_INTERVAL_MS 指定
 *
 * @param[in] handle
 *
